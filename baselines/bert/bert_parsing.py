...
def get_result_by_path_combine_type(path, golden_result, test_result,
    ...
    for file in files:
        print("开始测试:" + file)
        # 生成黄金结果
        file_golden = BertData(file, split_direction=SplitTypeEnum.right, global_reverse=global_reverse,
                               combine_type=combine_type)
        file_golden.shift_reduce_golden()
        file_golden.write_span_to_file(golden_result)
        # 生成测试结果
        file_test = BertData(file, split_direction=SplitTypeEnum.right, global_reverse=global_reverse,
                             combine_type=combine_type)
...
def load_data(model_file, config_path, checkpoint_path, num_classes):
    model = build_bert_model(config_path, checkpoint_path, num_classes=num_classes)
